...
#[tokio::test]
async fn test_source_generation_rate_with_slow_pulse_distance() {
    // This test verifies that the simulator's generation rate correctly
    // adheres to the configured `pulse_distance` and `eta` for the Source workflow.
    ...
    let mut sim = SimulatorBuilder::new()
        .with_hardware(hw)
        .with_rng(Pcg64Mcg::seed_from_u64(seed))
        .with_mode(SimulatorMode::Source) // Test the Source workflow
        .with_eta(eta)
        .with_qb_err(0.0)
        .with_angles(vec![0, 32, 64, 96])
        .with_modulator_state(ModulatorState::Random)
    ...
    // Create a dummy vector of GCs to pass to the function.
    // The values don't matter, only the length.
    let dummy_gcs: Vec<u64> = (0..batch_size as u64).collect();

    for i in 0..num_batches {
        println!("Generating source batch {}/{}", i + 1, num_batches);
        let angles_batch = sim
            .generate_angles_for_gcs(dummy_gcs.clone())
            .await
            .unwrap();
        assert_eq!(angles_batch.len(), batch_size);
    }
    ...
    println!(
        "Generated {} source events in {:?}. Expected time: {:?}",
        total_events, elapsed_time, expected_duration
    );

    let tolerance = Duration::from_millis(200).max(expected_duration / 10);
    assert!(elapsed_time >= expected_duration);
    assert!(elapsed_time <= expected_duration + tolerance);
}
